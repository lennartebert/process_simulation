"""Process simulation module including the process simulation model and helper functions"""
...
# Version of simulation model, ensures compatibility
VERSION = 0.04

# Metrics computed in aggregate functions
SIMULATION_METRICS = [
    "max_complexity",
    "mean_complexity",
    "has_phase_change",
...
class ProcessSimulationModel:
    """
    A graph-based process simulation model, including automation and exceptions.
    """

    def __init__(
        self, t, l, m, r, n, v_m, v_a=0, v_m_e=0.01, v_a_e=0.03, a=0, e=0, seed=None
    ):
        """
        Initialize the Process Simulation Model.

        Parameters:
            t (int): Number of simulation iterations.
            l (int): Number of activities (lexicon size).
            m (int): Number of subunits of the process, must be a factor of l.
            r (int): Size of the history matrix.
            n (int): Maximal sequence length factor (max sequence length is n * l).
            v_m (float): Random jump likelihood for non-exceptional cases at manual activities.
            v_a (float): Random jump likelihood for non-exceptional cases at automated activities (default: 0).
            v_m_e (float): Random jump likelihood for exceptional cases at manual activities (default: 0.01).
            v_a_e (float): Random jump likelihood for exceptional cases at automated activities (default: 0.03).
            a (float): Percentage of activities that are automated. (default: 0)
            e (float): Percentage of simulated cases that are exceptions. (default: 0)
    ...
    def get_v(self, activity, is_activity_automated, is_exception):
        """
        Get the random jump probability (variability) giving the current activity, whether the activity is automated and whether the case is an exception.
        """
        if (is_activity_automated[activity]) and (is_exception):
            return self.v_a_e
        elif (is_activity_automated[activity]) and (not is_exception):
            return self.v_a
        elif (not is_activity_automated[activity]) and (is_exception):
            return self.v_m_e
        elif (not is_activity_automated[activity]) and (not is_exception):
            return self.v_m
    ...
    def next_sequence(
        ...
        activity_modules,
        module_activities,
        is_activity_automated,
    ) -> list:
        """
        Generate the next sequence based on the current adjacency matrix and variation probabilities.

        Parameters:
            am (np.ndarray): Adjacency matrix representing transition probabilities (does not need to be normalized)
            global_source (int): Starting activity node.
            global_sink (int): Final activity node.
            module_sinks (list): List of module sink nodes.
            activity_modules (dict): Mapping of activity nodes to module sink nodes.
            module_activities (dict): Mapping of module sinks to their corresponding activity nodes.
            is_activity_automated (np.ndarray): Boolean array indicating whether activities are automated.

        Returns:
            list: New sequence of activities.
        """
        ...
        while (current_activity != global_sink) and (
            ...
            # do not vary from usual process if a random number is > variability
            elif np.random.rand() > self.get_v(
                current_activity, is_activity_automated, is_exception
            ):
                # go to the next node
                ...
                if is_activity_automated[current_activity]:
                    ...
                    # else, select the next node per weighted jump
                        ...
                        next_activity = np.searchsorted(
                            cumulative_occurences, random_value
                        )
    ...
    def run_simulation(self, normalize_adjacency_matrices=True) -> list:
        """
        Run the process simulation with parameters set in this model instance.

        Parameters:
            normalize_adjacency_matrices (bool, optional): If True, adjacency matrices are row-normalized before returning. (default: True)

        Returns:
            list: A list of adjacency matrices representing process transitions over time.
        """
        ...
        module_sinks = [
            math.ceil(self.l / self.m * sink) - 1 for sink in range(1, self.m + 1)
        ]
        # create a dictionary that maps each activity to a module
        activity_modules = {
            activity: module_sinks[int(self.m * activity / self.l)]
            for activity in range(0, self.l)
        }
        # create a dictionary that has all activities of a specific module
        module_activities = {
            sink_node: list(range(int(sink_node - self.l / self.m + 1), sink_node + 1))
            for sink_node in module_sinks
        }
        ...
        if normalize_adjacency_matrices:
            ...
            for am in historic_adjacency_matrices:
                ...
                norm_am = np.divide(
                    am,
                    sum_of_rows_matrix,
                    out=np.zeros_like(am),
                    where=sum_of_rows_matrix != 0,
                )
...
### Helper functions for graph metric computations


def summarize_simulation_run(adjacency_matrices):
    complexities = []
    max_complexity = None
    ...
    return {
        ...
        "has_phase_change": has_phase_change,
        "time_to_chaos": time_to_chaos,
    }
...
def summarize_am(
    ...
    metrics=[
        ...
        "density",
        "avg degree",
        "est. count simple paths",
    ],
    monte_carlo_max_sequence=None,
    ...
    if "depth" in metrics:
        depth = len(
            nx.algorithms.shortest_paths.generic.shortest_path(
                graph, source=min(list(graph)), target=max(list(graph))
            )
        )
        result["depth"] = depth
    ...
    if "est. count simple paths" in metrics:
        est_count_s_paths = 10 ** (
            0.08 + 0.08 * number_of_edges - 0.08 * number_of_nodes
        )
        result["est. count simple paths"] = est_count_s_paths

    if "count simple paths" in metrics:
        all_simple_paths = list(
            nx.all_simple_paths(graph, source=0, target=number_of_nodes - 1)
        )
    ...
    if (
        ("avg sequences ending in sink" in metrics)
        or ("avg sequences with loops" in metrics)
        or ("avg steps to sink" in metrics)
    ):
        monte_carlo_sequences = []

        for _ in range(monte_carlo_iterations):
            monte_carlo_sequences.append(
                get_random_walk_sequence(adjacency_matrix, max_sequence)
            )
        ...
        if "avg sequences ending in sink" in metrics:
            ...
            for sequence in monte_carlo_sequences:
                if (sequence[len(sequence) - 1]) == (len(adjacency_matrix) - 1):
                    sequence_count_ending_in_sink += 1

            result["avg sequences ending in sink"] = (
                sequence_count_ending_in_sink / monte_carlo_iterations
            )
        ...
        if "avg sequences with loops" in metrics:
            ...
            for sequence in monte_carlo_sequences:
                if len(sequence) != len(set(sequence)):
                    sequence_count_loop += 1

            result["avg sequences with loops"] = (
                sequence_count_loop / monte_carlo_iterations
            )
...
def summarize_ams(
    ...
    for adjacency_matrix in adjacency_matrices:
        am_summary = summarize_am(
            adjacency_matrix, metrics, monte_carlo_max_sequence, monte_carlo_iterations
        )
...
def get_random_walk_sequence(am, max_sequence=None):
    ...
    if num_rows != num_cols:
        raise Exception("Adjacency matrix not symmetrical")
    l = num_rows

    # set max_sequence if None
    if max_sequence is None:
        max_sequence = 5 * num_rows
    ...
    while len(sequence) < max_sequence and (not no_next_node):
        ...
        if sum(next_node_probabilities) == 0:
            ...
            next_activity = np.random.choice(
                list(range(l)), 1, p=next_node_probabilities
            )[0]
...
def get_heatmap_plot(
    ...
    fig, axes = plt.subplots(
        num_rows, num_columns, figsize=(num_columns * 3, num_rows * 3)
    )
...
def sample_adjacency_matrices(ams, sample_size):
    """
    Sample equally spaced adjacency matrices from a given list.

    Ensures the first and last matrices are always included and distributes the remaining samples evenly.

    Parameters:
        ams (list): List of adjacency matrices.
        sample_size (int): Number of matrices to sample from the list.

    Returns:
        dict: A dictionary where keys are the sampled indices (i.e., timesteps) and values are the corresponding adjacency matrices.

    Raises:
        ValueError: If sample_size is greater than the number of available matrices.
    """
    n = len(ams)
    if sample_size > n:
        raise ValueError(
            "sample_size cannot be greater than the number of entries in ams"
        )
...
def run_simulations_to_csv(
    ...
    if fill:
        ...
        aggregated_existing_results_df = aggregate_results_df(existing_results_df)
        ...
        for params in unique_params_list:
            ...
            try:
                simulation_runs_count = aggregated_existing_results_df.loc[
                    tuple(param_values), ("simulation_runs", "count")
                ]
            except KeyError:
                simulation_runs_count = 0
        ...
        expanded_params_list = [
            param
            for param in unique_params_list
            for _ in range(default_simulation_runs)
        ]
    ...
    with open(output_file_name, "a", newline="") as csvfile:
        ...
        with ProcessPoolExecutor() as executor:
            futures = {
                executor.submit(run_simulation_and_get_results, params): params
            ...
            for future in as_completed(futures):
                ...
                simulations_performed = len(futures) - len(
                ...
                simulations_performed_str = str(simulations_performed).zfill(
                    len(total_simulations_str)
                )
                if percent_progress < 1:
                    print(
                        f"Progress: {percent_progress:2.1%} | Simulations: {simulations_performed_str}/{total_simulations_str} | Estimated remaining time: {remaining_time_str}",
                        end="\r",
                        flush=True,
                    )
                else:
                    ...
                    print(" " * 200, end="\r")
                    print(
                        f"Progress: {percent_progress:2.1%} | Simulations: {simulations_performed_str}/{total_simulations_str} | Elapsed time: {time.strftime('%H:%M:%S', time.gmtime(elapsed_time))}"
                    )
...
def run_simulation_and_get_results(sim_params):
    ...
    simulation_result = summarize_simulation_run(adjacency_matrices)
...
def main(
    simulation_settings_path, output_file_name, random_order, sensitivity_analysis, fill
):
    """
    ...
    run_simulations_to_csv(
        simulation_settings, output_file_name, random_order, sensitivity_analysis, fill
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="local_results/results.csv",
